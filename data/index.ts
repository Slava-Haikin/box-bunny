...
class WeekMealPlanGenerator {
  ...
  private async generateMealPlan(): Promise<WeekMealPlan> {
    const periods = this.week.getMenuPeriods(
      this.workingWeekCookingStyle,
      this.weekendCookingStyle,
      true,
    );
  ...
  private async generateMenu(): Promise<Menu> {
    const breakfasts = await this.db
      .select()
      .from(recipesTable)
      .where(eq(recipesTable.meal, MEAL.breakfast));
    const lunches = await this.db
      .select()
      .from(recipesTable)
      .where(eq(recipesTable.meal, MEAL.lunch));
    const snacks = await this.db
      .select()
      .from(recipesTable)
      .where(eq(recipesTable.meal, MEAL.snack));
    const suppers = await this.db
      .select()
      .from(recipesTable)
      .where(eq(recipesTable.meal, MEAL.supper));
  ...
  private async deriveGroceryList(
    weekMealPlan: WeekMealPlan,
  ): Promise<GroceryList> {
  ...
  private async deriveRecipeIngredients(
    recipeId: number,
  ): Promise<(Ingredient & { quantity: number })[]> {
  ...
  private summarizeIngredients(
    ingredients: (Ingredient & { quantity: number })[],
  ): (Ingredient & { quantity: number })[] {
    const merged = ingredients.reduce<(Ingredient & { quantity: number })[]>(
      (acc, ingredient) => {
  ...
  private groupIngredientsByAisle(
    ingredients: (Ingredient & { quantity: number })[],
  ) {
    return Object.entries(
      ingredients.reduce<GroceryGroups>((acc, ingredient) => {
...
export const cachedWeekMealPlan = (
  ...args: [CookingStyle.Lazy, CookingStyle.Lazy, boolean]
) => {
