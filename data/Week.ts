...
export default class Week {
  ...
  getMenuPeriods(
    workingStyle: CookingStyle,
    weekendStyle: CookingStyle,
    ...
    const workingMenuCount = Math.ceil(
      workingDays.length / deriveMenuUpdateInterval(workingStyle),
    );
    ...
    const weekendMenuCount = Math.ceil(
      weekendDays.length / deriveMenuUpdateInterval(weekendStyle),
    );
    const weekendMenuLength = Math.ceil(weekendDays.length / weekendMenuCount);
    const weekendChunks = weekendIncluded
      ? splitDates(weekendDays, weekendMenuLength)
      : [];
